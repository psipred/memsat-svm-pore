...
void Topology::Print(){
	...
	/*
	//cout << "Search for pore-lining re-entrant helices:" << endl;
	found_pore = 0;
	string pore_lining_re_helix_string;
	for (unsigned int h = 0; h < re_helix_vector.size(); h += 4){
			//cout << 1+(h/2) << ": " << re_helix_vector[h+1] << " --- " << re_helix_vector[h+2];
			float pore_lining_ave = 0;
			unsigned int pore_lining_residues = 0;
			for (int p = re_helix_vector[h+1]-1; p <= re_helix_vector[h+2]-1; p++){
				pore_lining_ave+=svm_pl[p];
				if(svm_pl[p] > 0){
					//cout << p << " score " << svm_pl[p] << endl; 
					pore_lining_residues++;
			...
			pore_lining_ave /= 1+re_helix_vector[h+1]-re_helix_vector[h];
			cout << "\tPore lining re-entrant: " << pore_lining_ave << " " << pore_lining_residues << endl;
			if(pore_lining_ave >= pore_threshold){
			//if(pore_lining_residues >= (unsigned int)pore_threshold){
				//cout << "\tPore lining: " << pore_threshold << endl;
				...
				string s1;
				stringstream b1;
				b1 << re_helix_vector[h+1];
				s1 = b1.str();
				string s2;
				stringstream b2;
				b2 << re_helix_vector[h+2];
				s2 = b2.str();	
				...
				if (format == 3){
					...
					if (h != 0){
						...
						if (n_term == "in"){
							if ((h/2) % 2){
								pore_lining_re_helix_string.append("o");
								suffix.append("i");
							}else{
								pore_lining_re_helix_string.append("i");
								suffix.append("o");
							}						
						}else{
							if ((h/2) % 2){
					...
					pore_lining_re_helix_string.append(s1);
					pore_lining_re_helix_string.append("-");
					pore_lining_re_helix_string.append(s2);
					pore_lining_re_helix_string.append(suffix);
				...
				// A.6,21;B.40,57;C.142,172;D.214,236;E.276,302	
					...
					pore_lining_re_helix_string += letters[h/2];
					pore_lining_re_helix_string.append(".");
					pore_lining_re_helix_string.append(s1);
					pore_lining_re_helix_string.append(",");
					pore_lining_re_helix_string.append(s2);
